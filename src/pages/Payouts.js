...
export default function Payouts() {
  ...
  const calculateSummary = useCallback(() => {
    ...
    const totalRedeemRequests = redeemRequests.length;
    const processingRedeems = redeemRequests.filter(r => r.redeemStatus === 'processing').length;
    const depositedRedeems = redeemRequests.filter(r => r.redeemStatus === 'deposited').length;
  ...
  }, [payouts, wallets, redeemRequests]);
  ...
  useEffect(() => {
    ...
    fetchWallets();
    fetchRedeemRequests();
  }, [fetchPayouts, fetchWallets, fetchRedeemRequests]);
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Redeem Requests</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{summary.totalRedeemRequests}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {summary.processingRedeems} processing, {summary.depositedRedeems} deposited
              </p>
            </div>
            <div className="p-3 bg-orange-100 dark:bg-orange-900 rounded-full">
              <WalletIcon className="w-6 h-6 text-orange-600 dark:text-orange-400" />
            </div>
          </div>
        </div>
